...
`checkSetup` を実行すると、設定・カレンダー接続・Chat 接続・トリガー・読み取りルールのテストをまとめて点検できます。

```
===== chat-to-calendar セットアップ点検 =====
✅ 設定の読み込み OK
✅ スクリプトのタイムゾーン: Asia/Tokyo（設定: Asia/Tokyo）
✅ カレンダー「primary」にアクセスできます
✅ Google Chat に接続できます（参加スペース 5 件）
⚠️ 自動実行トリガー: 0 件（installTriggers を実行してください）
✅ 読み取りルールのテスト 33/33
```
...
| 時間帯の言い方 | `朝イチ`(8:00) `午前中`(9:00-12:00) `午後イチ`(13:00) `夕方`(17:00) `夜`(19:00) |
| 所要時間 | `10時から30分` `10:00から1時間` |
| 繰り返し | `毎週月曜` `隔週火曜` `毎月15日` `毎日` `平日` |
| 場所 | `@事務所` `場所:現場事務所` |
...
| 関数 | 用途 |
|---|---|
| `checkSetup` | 設定・接続・トリガー・読み取りルールをまとめて点検する |
| `previewOnly` | 登録せずに「何が拾われるか」だけログに出す（取り込み位置も進めない） |
| `syncNow` | 1 回分の取り込みを手動実行する |
| `installTriggers` / `removeTriggers` | 自動実行トリガーの作成・削除 |
| `listMySpaces` | 参加中のスペース名を一覧表示する |
| `resetState` | 取り込み位置と処理済み記録を消して読み直せるようにする |
| `runParserTests` | 読み取りルールのテストを実行する |
...
- 別のメッセージで同じ予定が流れた場合に備えて、**同じ時間帯に同じ件名の予定がすでにあれば登録しません**
  （`skipIfSimilarEventExists`）。
...
| `allowRecurring` | `true` | `毎週月曜` などを繰り返し予定として登録する |
| `recurrenceCounts` | 毎週26/毎月12/毎日60 | 繰り返しを何回分作るか |
| `skipIfSimilarEventExists` | `true` | 同じ時間帯に同名の予定があれば登録しない |
...
まず `checkSetup` を実行してください。設定・接続・トリガーをまとめて点検し、どこで止まっているかを表示します。
...
- 繰り返し予定は「毎週◯曜／隔週◯曜／毎月◯日／毎日／平日」に対応します。
  「第2火曜」「毎週月水金」のような指定は読み取れません。
